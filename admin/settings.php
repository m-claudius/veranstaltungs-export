...
function kse_register_settings() {
    ...
    add_settings_field('cron_burg', 'Burg Seevetal', function(){
        $o = get_option('kse_options', []);
        $val = $o['cron_burg'] ?? 'disabled';
        kse_render_cron_select('kse_options[cron_burg]', $val);
        kse_render_cron_next('kse_cron_burg');
        if (function_exists('kse_render_run_now_link')) kse_render_run_now_link('burg');
}, 'kse_settings', 'kse_sec_cron');
...
function kse_sanitize_options($in) {
    ...
    $out['cron_empore']  = in_array(($in['cron_empore']  ?? 'disabled'), $allow, true) ? $in['cron_empore']  : 'disabled';
    $out['cron_winsen']  = in_array(($in['cron_winsen']  ?? 'disabled'), $allow, true) ? $in['cron_winsen']  : 'disabled';
    $out['cron_burg'] = in_array(($in['cron_burg'] ?? 'disabled'), $allow, true) ? $in['cron_burg'] : 'disabled';
    ...
    kse_reschedule_cron('kse_cron_burg',     $out['cron_burg']);
